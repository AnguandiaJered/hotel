...
export const SaveAffectation = () => {
  ...
  const {refpersonnel,refservice,dateaffectation,montant,devise,libelle,author} = affectation;
    ...
    const onSubmit = async e =>{
      e.preventDefault();
      await axios.post('http://localhost:8080/affectation/create',affectation).then(res =>{
  ...
  return (
    <Fragment>
      <Link to='#' variant="outlined" className='btn btn-primary' onClick={handleClickOpen}>
        <i className='fa fa-plus'></i> Add Affectation
      </Link>
      <BootstrapDialog
        onClose={handleClose}
